...
export async function createOrder({
  ...
  try {
    ...
    if (!journalNumber && user.paymentMethod === 'E-Transaction') {
      return {
        success: false,
        message: 'Journal Number is required for E-Transaction',
        redirectTo: '/payment-method',
      };
    }
...
export async function getOrderById(orderId: string) {
  const data = await prisma.order.findFirst({
    where: { id: orderId },
    include: {
      orderItems: true, // Use correct relation name
...
export async function getAllOrders({
  ...
  // Add filter for searching by buyer's name or journal number
  if (query && query !== 'all') {
    queryFilter.OR = [
      {
        user: {
